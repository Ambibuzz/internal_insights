...
class BigQueryDatabase(BaseDatabase):
    ...
    def get_table_preview(self, table, limit=100):
        data = self.execute_query(f"""select * from {self.project_id}.{table} limit {limit}""", cached=True)
        length = self.execute_query(f'''select count(*) from {self.project_id}.{table}''', cached=True)[0][0]
        return {
            "data": data or [],
            "length": length or 0,
        }
